...
class Player(object):
    def __init__(self, riot_id, auth_token, entitlement_token, puuid, shard="na"):
        ...
        self.vp, self.radianite, self.kingdomcredits = self.get_currencies(
            self.auth_token, self.entitlement_token, self.puuid
        )
        self.skins = []
    ...
    def get_currencies(auth_token, entitlement_token, puuid, shard="na") -> tuple:
        ...
        with requests.Session() as s:
            r = s.get(
                f"https://pd.{shard}.a.pvp.net/store/v1/wallet/{puuid}", headers=headers
            )
            d = r.json()["Balances"]
        return (
            d["85ad13f7-3d1b-5128-9eb2-7cd8ee0b5741"],
            d["e59aa87c-4cbf-517a-5983-6e81511be9b7"],
            d["85ca954a-41f2-ce94-9b45-8ca3dd39a00d"],
        )
    ...
    def get_shop(self) -> list[SkinAsset]:
        with requests.Session() as session:
            ...
            body = [self.puuid]
            r = session.get(
                f"https://pd.{self.shard}.a.pvp.net/store/v2/storefront/{self.puuid}",
                headers=headers,
                json=body,
            )
        ...
        self.skins = [
            SkinAsset.from_uuid(skinuuid) for skinuuid in data["SingleItemOffers"]
        ]
        for skin in self.skins:
            skin.price = data["SingleItemStoreOffers"][self.skins.index(skin)]["Cost"][
...
def send_webhook(content="‎", embed: DiscordEmbed = None):
    ...
    webhook = DiscordWebhook(
        config["discordWebhookUrl"], username="Valorant Shop Alerts"
    )
    if embed:
        ...
        for skin in account.get_shop():
            if not handle_skin(skin):
                ...
                expires_in = datetime.datetime.now() + datetime.timedelta(
                    seconds=account.shop_expiry_time
                )
                e.add_embed_field(
                    name="Expires in", value=f"<t:{round(expires_in.timestamp())}:R>"
                )
...
def handle_accounts(accounts=list[dict]) -> list[Player]:
    ...
    for account in accounts:
        ...
        try:
            ...
            send_webhook(
                f"{username} has incorrect login information or 2fa(not supported currently)."
            )
            continue
